...
public:
  ...
  StlIntVector charStateVector(Node v_ci) const
  {
    StlIntVector res(_S.size(), 0);
    ...
    while (v_ci != _root)
    {
      ...
      if (res[ci.first] == 0)
      {
        res[ci.first] = ci.second;
      }
      assert(InArcIt(_T, v_ci) != lemon::INVALID);
      v_ci = _T.source(InArcIt(_T, v_ci));
    }
